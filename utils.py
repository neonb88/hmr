...
def preprocess_img_HMR(fname):
    img=np_img(fname)
    mask=seg(img)
    cropped,_=crop_person(img,mask)
    dot_loc=fname.rfind('.')
    img_type=fname[dot_loc:]
    scipy.misc.imsave(fname[:dot_loc]+'_cropped'+img_type, cropped)
    ...
    pltshow(cropped)
    targ_height=150.
    targ_width=img.shape[1]   * targ_height/img.shape[0]

    img_150_pix_height=np.array(Image.fromarray(cropped).resize((int(round(targ_width)), int(round(targ_height)))))
    pltshow(img_150_pix_height)
    scipy.misc.imsave(fname[:dot_loc]+'_150_pix_height'+img_type,img_150_pix_height)
    return img_150_pix_height


if __name__=='__main__':
    preprocess_img_HMR(sys.argv[1])
    #manual_vid_cut()
